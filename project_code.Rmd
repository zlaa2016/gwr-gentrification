...
author: "Abby Lewis & Zining Tenny Liu"
date: "4/23/2019"
...
In the first section, I load the data and create four maps. The first two are maps of starbucks locations in california and the world respectively. The last two are the same data, but with a background map that shows state/country boundaries.
```{r}
...
In this section, I take in data from the census api and make three figures that overlay a given aspect of census data with starbucks locations. My first graph is a graph of median county-level income in the US, then I subset this to California, and finally I do the number of white population in counties in California. If we want to incorporate the final graph or any other metrics related to race, we should probably divide by the total population in the census tract. I haven't done that so far. 
```{r}
...
In this section, I load census data for Wisconsin and use the previous linear model to predict county-wide Starbucks abundance.
```{r}
